 ...
 * @author lorinpa
 ...
 Note!   We should have between 3 and 5 book categories for the duration of this suite.
 DbUtil creates 4 book-categories.  However, we have an add test and a delete  test.
 We don't know what sequence the tests are going to run in. Therefore, if the delete  test runs first,
 the database will only have 4 book categories remaining. Likewise, if the "add " test runs first, 
 we will have 5  book categories in the database. 
 
 Our delete  test deletes the 2nd generated book-category.. 
...
public class TestBooKCategoryController {
    ...
    /*
     *  Tests that the controller dispatches a view containing at least 4 book categores (the number of records generated by DbUtil minus one 
     *  -- in case delete test has run).
     *  Tests that one of the   book category  is indeed the first book category (preslected during setup).
     *
